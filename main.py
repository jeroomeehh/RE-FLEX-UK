...
def sign_request(private_key: ec.EllipticCurvePrivateKey, path: str):
    ...
    if time.time() * 1000 > key_id["expiration"] - 60000:
...
def get_offer_list(rejected_ids: set):
    ...
    response = amz_request(
        method="post", 
        url="https://flex-capacity-eu.amazon.com/GetOffersForProviderPost",
...
def accept_block(block):
    ...
    accept = amz_request(
        method="post", 
        url="https://flex-capacity-eu.amazon.com/AcceptOffer",
...
def validate_captcha(validationId: str) -> bool:
    ...
    res = amz_request(
        method="post",
        url="https://flex-capacity-eu.amazon.com/ValidateChallenge",
        json=data,
        session=session,
        sign_request=True
    )
